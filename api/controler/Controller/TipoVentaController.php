...
class TipoVentaController extends AppController{
	public function tipo_venta_fn($key,$sv600_id,$funcion)
	{
		...
		try
		{
			...
			if($rows >=1)
			{
				...
				if ($rows>=1)
				{
					...
					$resultado = $link->query("select * from tipo_venta");
					$rows=count($resultado);
					...
					{
						$respuesta= '{"error":-1,"message":"no existen informacion de tipos de venta"}';
					}
				}
				else
				{
