...
class MotivosController extends AppController{
	public function motivo_fn($key,$sv600_id,$funcion)
	{
		...
		try
		{
			...
			if($rows >=1)
			{
				...
				if ($rows>=1)
				{
					...
					$resultado = $link->query("select * from motivos");
					$rows=count($resultado);
					...
					if ($rows>=1)
					{
						...
						foreach($resultado as $valores)
						{
							foreach($valores as $valor)
							{
								if ($respuesta=="")
								{
									$respuesta='{"motivo":"'.$valor['motivo'].'","descripcion":"'.$valor['descrip'].'"}';
								}
								else
								{
									$respuesta.=',{"motivo":"'.$valor['motivo'].'","descripcion":"'.$valor['descrip'].'"}';
					...
					{
						$respuesta= '{"error":-1,"message":"no existen informacion de motivos de no surtido"}';
					}
				}
				else
				{
