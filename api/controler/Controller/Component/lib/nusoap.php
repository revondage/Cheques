	...
	function serialize($debug = 0)
	{
			...
			foreach($this->messages as $msgName => $msgParts) {
				...
				if(is_array($msgParts)){
					foreach($msgParts as $partName => $partType) {
						...
						$xml .= "\n" . '  <wsdl:part name="' . $partName . '" ' . $elementortype . '="' . $typePrefix . ':' . $localPart . '" />';
					}
				}
				$xml .= '</wsdl:message>';
			} 
		...
		if (count($this->bindings) >= 1) {
			...
			foreach($this->bindings as $bindingName => $attrs) {
				$binding_xml .= "\n<wsdl:binding name=\"" . $bindingName . '" type="tns:' . $attrs['portType'] . '">';
				$binding_xml .= "\n" . '  <soap:binding style="' . $attrs['style'] . '" transport="' . $attrs['transport'] . '"/>';
				$portType_xml .= "\n<wsdl:portType name=\"" . $attrs['portType'] . '">';
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					$binding_xml .= "\n" . '    <wsdl:input><soap:body use="' . $opParts['input']['use'] . '" namespace="' . $opParts['input']['namespace'] . '"' . $enc_style . '/></wsdl:input>';
					if (isset($opParts['output']['encodingStyle']) && $opParts['output']['encodingStyle'] != '') {
						$enc_style = ' encodingStyle="' . $opParts['output']['encodingStyle'] . '"';
					} else {
						$enc_style = '';
					}
					$binding_xml .= "\n" . '    <wsdl:output><soap:body use="' . $opParts['output']['use'] . '" namespace="' . $opParts['output']['namespace'] . '"' . $enc_style . '/></wsdl:output>';
					$binding_xml .= "\n" . '  </wsdl:operation>';
					$portType_xml .= "\n" . '  <wsdl:operation name="' . $opParts['name'] . '"';
					if (isset($opParts['parameterOrder'])) {
					    $portType_xml .= ' parameterOrder="' . $opParts['parameterOrder'] . '"';
					} 
					$portType_xml .= '>';
					if(isset($opParts['documentation']) && $opParts['documentation'] != '') {
						$portType_xml .= "\n" . '    <wsdl:documentation>' . htmlspecialchars($opParts['documentation']) . '</wsdl:documentation>';
					}
					$portType_xml .= "\n" . '    <wsdl:input message="tns:' . $opParts['input']['message'] . '"/>';
					$portType_xml .= "\n" . '    <wsdl:output message="tns:' . $opParts['output']['message'] . '"/>';
					$portType_xml .= "\n" . '  </wsdl:operation>';
				} 
				$portType_xml .= "\n" . '</wsdl:portType>';
				$binding_xml .= "\n" . '</wsdl:binding>';
		...
		if (count($this->ports) >= 1) {
			foreach($this->ports as $pName => $attrs) {
				$xml .= "\n" . '  <wsdl:port name="' . $pName . '" binding="tns:' . $attrs['binding'] . '">';
