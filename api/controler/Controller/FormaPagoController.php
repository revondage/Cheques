<?php
class FormaPagoController extends AppController{
	public function forma_pago_fn($key,$sv600_id,$funcion)
	{
		...
		try
		{
			...
			if($rows >=1)
			{
				...
				if ($rows>=1)
				{
					...
					$resultado = $link->query("select * from forma_pago");
					$rows=count($resultado);
					...
					if ($rows>=1)
					{
						...
						foreach($resultado as $valores)
						{
							foreach($valores as $valor)
							{
								if ($respuesta=="")
								{
									$respuesta='{"id":"'.$valor['id'].'","descripcion":"'.$valor['descripcion'].'"}';
								}
								else
								{
									$respuesta.=',{"id":"'.$valor['id'].'","descripcion":"'.$valor['descripcion'].'"}';
					...
					{
						$respuesta= '{"error":-1,"message":"no existen informacion de formas de pago"}';
					}
				}
				else
				{
